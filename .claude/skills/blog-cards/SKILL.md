...
한 글 안에서 카드마다 배경 느낌이 달라지도록 템플릿마다 고유 배경 정체성을 둔다. 같은 팔레트 안에서도 `base` 다크, `accent` 컬러풀, `highlight` 라이트, split, terminal 등 다른 결로 분배된다. 한 글에서 카드 4~5장이 나란히 놓일 때 색감이 단조롭지 않게 템플릿을 섞어 고른다.
