...
## Purpose
Claude Code 기반 워드프레스 블로그 글 자동 생성 파이프라인. 주제 입력 → 대화형 웹 리서치(Tavily) + 질의응답 → 마크다운 초안 → 자가 검수 → HTML 카드 템플릿 이미지 생성까지 연결된다. 필자별 컨셉은 `profiles/<name>.yml` 로 공유 포맷(D형) 관리. 생성물은 `posts/` 하위에 저장되며 워드프레스 업로드는 수동(블록 에디터 붙여넣기).
...
- 워드프레스 업로드는 수동(블록 에디터 붙여넣기). REST API 자동화는 현재 범위 밖.
